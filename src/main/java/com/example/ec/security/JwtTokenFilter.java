...
public class JwtTokenFilter extends GenericFilterBean {
     ...
     * If it is valid then set the currnt context with the Authentication(user and roles) found on the token
    ...
    public void doFilter(ServletRequest req, ServletResponse res, FilterChain filterChain) throws IOException, ServletException {
        LOGGER.info("doFilter - Process request to check for a JSON Web Token ");
        // Check for Authorization:Bearer JWT
        String headerValue = ((HttpServletRequest) req).getHeader("Authorization");
        getBearerToken(headerValue).ifPresent(token-> {
            LOGGER.info("doFilter - token is [" + token + "]");
            ...
            userDetailsService.loadUserByJwtToken(token).ifPresent(userDetails -> {
                //Add the user details (Permissions) to the Context for just this API invokation
    ...
    private Optional<String> getBearerToken(String headerVal) {
        LOGGER.info("getBearerToken - have header [" + headerVal + "]");
        if(headerVal != null && headerVal.startsWith(BEARER)) {
            LOGGER.info("getBearerToken - bearer found");
